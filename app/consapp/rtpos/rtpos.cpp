...
#define MAXCON 32                          /* 控制台最大数量 */
#define MAXARG 10                          /* 命令中参数的最大数量 */
#define MAXCMD 256                         /* 命令的最大长度 */
#define MAXSTR 1024                        /* 流的最大长度 */
#define OPTSDIR "."                        /* 默认配置目录 */
#define OPTSFILE "rtpos.conf"              /* 默认配置文件 */
#define NAVIFILE "rtpos.nav"               /* 导航保存文件 */
#define STATFILE "rtpos_%Y%m%d%h%M.stat"   /* 解算状态文件 */
#define TRACEFILE "rtpos_%Y%m%d%h%M.trace" /* 调试跟踪文件 */
#define LOGFILE "rtpos_%Y%m%d%h%M.log"     /* 守护进程日志文件 */
#define INTKEEPALIVE 1000                  /* 保持连接间隔(ms) */

#define ESC_CLEAR "\033[H\033[2J" /* ansi/vt100转义: 清除屏幕 */
#define ESC_RESET "\033[0m"       /* ansi/vt100: 重置属性 */
#define ESC_BOLD "\033[1m"        /* ansi/vt100: 粗体 */

#define SQRT(x) ((x) <= 0.0 || (x) != (x) ? 0.0 : sqrt(x))

/* 类型定义 ----------------------------------------------------------*/

typedef struct {    /* 控制台类型 */
  int state;        /* 状态 (0:停止,1:运行) */
  vt_t *vt;         /* 虚拟终端 */
  pthread_t thread; /* 控制台线程 */
} con_t;
...
static int timetype = 0;              /* 时间格式 (0:gpst,1:utc,2:jst,3:tow) */
static int soltype = 1;               /* 解算格式 (0:dms,1:deg,2:xyz,3:enu,4:pyl) */
static int solflag = 3;               /* 解算标志 (1:std+2:age/ratio/ns) */
static int strtype[] = {STR_SERIAL, STR_NONE, STR_NONE, STR_NONE,
                        STR_NONE,   STR_NONE, STR_NONE, STR_NONE};                 /* 流类型 */
static char strpath[8][MAXSTR] = {"", "", "", "", "", "", "", ""};                 /* 流路径 */
static int strfmt[] = {STRFMT_UBX, STRFMT_RTCM3, STRFMT_SP3, SOLF_LLH, SOLF_NMEA}; /* 流格式 */
static char rcvopt[3][256] = {""};                                                 /* 接收机选项 */
static int svrcycle = 10;              /* 服务器周期 (ms) */
static int timeout = 10000;            /* 超时时间 (ms) */
static int reconnect = 10000;          /* 重连间隔 (ms) */
static int nmeacycle = 5000;           /* nmea请求周期 (ms) */
static int buffsize = 32768;           /* 输入缓冲区大小 (bytes) */
static int navmsgsel = 0;              /* 导航信息选择 */
static char proxyaddr[256] = "";       /* http/ntrip代理 */
static int nmeareq = 0;                /* nmea请求类型 (0:关闭,1:纬度/经度,2:单点) */
static double nmeapos[] = {0, 0, 0};   /* nmea位置 (纬度/经度/高度) (度,m) */
static char rcvcmds[3][MAXSTR] = {""}; /* 接收机命令文件 */
#ifdef RTKSHELLCMDS
static char startcmd[MAXSTR] = ""; /* 启动命令 */
static char stopcmd[MAXSTR] = "";  /* 停止命令 */
#endif
static int modflgr[256] = {0};  /* 接收机选项修改标志 */
static int modflgs[256] = {0};  /* 系统选项修改标志 */
static int moniport = 0;        /* 监控端口 */
static int keepalive = 0;       /* 保持连接标志 */
...
#define FMTOPT                                                                                    \
  ...
  "14:unicore,15:rinex,16:sp3,17:clk"
...
static void prtime(vt_t *vt, gtime_t time) {
  ...
  if (timetype == 1) {
    ...
    snprintf(tstr, sizeof(tstr), "  %04d %9.2f", week, tow);
  } else
...
static void prnavidata(vt_t *vt) {
  ...
  for (i = 0; i < MAXSAT; i++) {
    ...
    if (!(sys & (SYS_GPS | SYS_GAL | SYS_QZS | SYS_CMP)) || eph[i].sat != i + 1) continue;
    // Mask QZS LEX health.
  ...
  for (i = 0; i < MAXSAT; i++) {
    ...
    vt_printf(vt, "%3s %3s %3d %3d %3d %3d  %02X %19s %19s %19s %3d %3d\n", id, valid ? "OK" : "-",
              geph[prn - 1].iode, 0, geph[prn - 1].frq, geph[prn - 1].age, geph[prn - 1].svh, s1,
              "-", s2, 0, 0);
  }
...
static void prstream(vt_t *vt) {
  ...
  const char *type[] = {"-",   "serial", "file",     "tcpsvr", "tcpcli", "ntrips", "ntripc",
                        "ftp", "http",   "ntripcas", "udpsvr", "udpcli", "membuf"};
...
static void cmd_mark(char **args, int narg, vt_t *vt) {
  ...
  if (narg > 1) snprintf(markername, sizeof(markername), "%s", args[1]);
  if (narg > 2) snprintf(markercomment, sizeof(markercomment), "%s", args[2]);
...
static void cmd_mode(char **args, int narg, vt_t *vt) {
  ...
  if (args[1][0] == 'G' || args[1][0] == 'g') {
    // Go
    if (pmode == PMODE_KINEMA || pmode == PMODE_STATIC || pmode == PMODE_STATIC_START ||
        pmode == PMODE_FIXED)
      pmode = PMODE_KINEMA;
  ...
  while (con->state) {
    ...
    switch (j) {
      ...
      case 12:
        ...
        break;
      case 13:
        cmd_mark(args, narg, con->vt);
        break;
      case 14:
        cmd_mode(args, narg, con->vt);
        break;
      case 15:
